...
pid_t ft_exec_1(char **args, int (*pipe_fd)[2])
{
	...
	if (pid == 0)
	{
		...
		close(pipe_fd[2][READ_END]);
		close(pipe_fd[2][WRITE_END]);
...
pid_t ft_exec_3(char **args, int (*pipe_fd)[2])
{
		...
		if (pid == 0)
	{
		...
		dup2(pipe_fd[2][WRITE_END],STDOUT_FILENO);
		close(pipe_fd[2][WRITE_END]);
...
pid_t ft_exec_4(char **args, int (*pipe_fd)[2])
{
	pid_t pid = fork();
		if (pid == 0)
	{
		dup2(pipe_fd[2][READ_END], STDIN_FILENO);
		close(pipe_fd[2][READ_END]);
...
int main(void)
{
	...
	char *arg3[3]={"/bin/grep", "git", NULL};
	char *arg4[3]={"/bin/wc", "-l", NULL};
	int pipe_fd[3][2];
	pid_t pid[4];
